...
public class DBApp {
	...
	public void insertIntoTable(String strTableName, 
						...
						//  3) normal ({rakameltuple, lastpage1, lastpage, rakamelpage})
									...
									if (binarySearchReturn[0] instanceof String) {
										...
										String ourinsertedrow = firstPage.addTuple(tupletoinsert);  //return string won't be used
										//serialize the page

									} else if (binarySearchReturn[0] instanceof Integer) {
										if((int)binarySearchReturn[0] == -1){
											insertinpageskonato(t, tupletoinsert,0, -1);
										}
										else{
											...
											insertinpageskonato(t, tupletoinsert, rakamelpage, rakameltuple);
										}
								...
								//recall: Hashtable<String,Index> hash = t.getIndexHash();  //column name , Index
									...
									//table has an index on at least 1 column
									...
									//table has an index on the primary key column --> we'll use it in the insertion
									   ...
									   bplustree tree = clusteringindex.getBTree();
									   Object VTpInBt = tupletoinsert.getValue(primarykey);
									   Comparable comp = (Comparable) VTpInBt;
									//    tree.insert(comp, tupletoinsert); // TODO tupletoinsert is add as tuple not key?

									   // 2 steps: 1) insert into the page using the other indexes to find our loc.	2) insert into the index



									   //		1) insert into the page using the other indexes to find loc.		//





									   //			2) insert into all indexes we have (we are sure they include PK index)			//
									...
									//the index is not on the primary key column
									// normal insertion into the page (copied from above)
									//Note:  we still need to insert in the indexes we have

									Object [] binarySearchReturn = binarysearchkonato(t, tupletoinsert);

									// 3 return cases:  1) string: this will be the first tuple to insert
									//  2) contains -1 : my tuple's PK is the smallest PK 
									//  3) normal ({rakameltuple, lastpage1, lastpage, rakamelpage})
												Page firstPage;
												if (binarySearchReturn[0] instanceof String) {
													...
													String ourinsertedrow = firstPage.addTuple(tupletoinsert);
												...
												while(flagx){
													if (keys.hasMoreElements()) {
														...
														bplustree tree = firstValue.getBTree();
														// tree.insert(firstKey, firstValue); TODO firstvalue is add as index?
														flagx = true;
	...
	public static void insertinpageskonato(Table table, Tuple tuple, int rakamelpage, int rakameltuple){
		Vector<String> pageFileNames = table.getPageFileNames();
					...
					if(tuples.size() == (page1.getMaxRows()+1)){
						if(rakamelpage == (pageFileNames.size()-1)){  //check if it's the last page
							try{
								...
								tuples.remove(tuples.size()-1); //removing the extra tuple from our full page (not the new created one)
								tuples.add(tuple);
						...
						String oldpage = pageFileNames.get(rakamelpage);
						// Serializer.serialize( ,oldpage);
						...
						try {
							...
							nextpage1 = (Page) Serializer.deSerialize(nextpage);
						
						} catch (Exception e) {
					
							return;
						}
						 tuples = nextpage1.getTuples();
						 newindex = 0;
					}
					else{
						flag = false;
