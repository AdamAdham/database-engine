...
public class bplustree {
	 ...
	 */
	public HashMap<Comparable,Vector<String>> search(Comparable lowerBound, Comparable upperBound,boolean lowerBoundInclusive,boolean upperBoundInclusive) {

		// Instantiate Double array to hold values
		HashMap<Comparable,Vector<String>> values = new HashMap<Comparable,Vector<String>>();
	...
	public class LeafNode extends Node {
		...
		public boolean insertOld(DictionaryPair dp) {
			// System.out.println("printing befor");
			// 	for(int j=0;j<dictionary.length;j++){
			// 		if(dictionary[j]!=null){
			// 			System.out.println("index: "+ j);
			// 			System.out.println("key: "+ dictionary[j].key);
			// 			System.out.println("value: "+ dictionary[j].value);
			// 		}
			// 	}
			if (this.isFull()) {
				...
				// int index = binarySearch(dictionary, maxNumPairs, dp.key);
				int index=-1;
				int i = 0;
				...
				if(index<0){
					...
					this.dictionary[index].value.add(dp.value.get(0)); // TODO if i have time change that the entered arguement is a string not a pair
				}
	...
	public static String helperforindexsearch(bplustree bp, Comparable comp){
	   ...
	   for(int i=0;i<ln.dictionary.length;i++){
		   if(ln.dictionary[i]==null){
				...
				System.out.println(ln.dictionary[i]);
			}	
		}
		for(int i=0;i<ln.dictionary.length;i++){
			if(ln.dictionary[i]==null) break;
			if(ln.dictionary[i].key==comp){     // not sure if that is the correct way to compare the comparables
				index=i;
				break;
			}
		}
		...
		if(index==ln.dictionary.length-1 || ln.dictionary[index+1]==null){
			// If at end of node || the right of key at that node is null
	   // 	// We got to the right sibling
		   LeafNode rightSibling = ln.rightSibling;
			index = -1;
			ln = rightSibling;
		}
	   //ln.dictionary[index].value
		//System.out.println(index);
	   
		String page = ln.dictionary[index+1].value.get(0);
		bp.delete(comp);
		return page;
		//Comparable key = ln.dictionary[index+1].key;
	}
	...
	// System.out.println(bp.search("d","v"));
	// System.out.println(bp.search("a"));
	// bp.delete("a");
	// System.out.println(bp.search("a"));
	 LeafNode ln = bp.insert("e", "");
	 int index=-100;
	for(int i=0;i<ln.dictionary.length;i++){
		if(ln.dictionary[i]==null){
	...
	// System.out.println(bp.search("d"));

	System.out.println(bp.search("d",null,true,true));
